...
class Solver(object):
    ...
    def __init__(self, reid_loader, config):
        ...
        self.show_example = config.show_example
    ...
    def train(self):
        ...
        for i in range(self.c_dim):
            ...
            c_fixed_list.append(c_trg.to(self.device))
        ...
        for i in range(start_iters, self.num_iters):
            ...
            if (i+1) % self.sample_step == 0:
                with torch.no_grad():
                    ...
                    if self.show_example:
                        self.vis.images(self.denorm(x_concat.data.cpu()), nrow=1, opts=dict(title='iter_' + str(i+1)), win=i % 6)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (x_real, c_org, name_real) in enumerate(data_loader):
                ...
                for jj in range(len(x_fake_list)):
                    ...
                    for kk in range(len(name_real)):
                        if jj == 0:
                            continue
                        else:
                            # save_name = name_real[kk][:-4] + '_fake_' + str(c_org[kk] + 1) + 'to' + str(jj) + '.jpg'
                            save_name = name_real[kk][:-4] + '_fake_' + str(jj) + '.jpg'
                ...
                print('Translated test images and saved into {}..!'.format(self.result_dir))
